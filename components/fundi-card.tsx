    ...
    <Card className="group overflow-hidden border-border/70 bg-linear-to-b from-card via-card to-muted/20 transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10">
      <CardHeader className="pb-3">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">{fundi.reviews}+ jobs</span>
        </div>
        <div className="flex gap-3">
          ...
          <div className="grow">
            <div className="flex items-start justify-between gap-2">
              <div>
                <CardTitle className="text-lg leading-tight tracking-tight">{fundi.name}</CardTitle>
                <CardDescription className="mt-1 text-sm">
                  {fundi.title}
                </CardDescription>
              </div>
              {fundi.premiumLevel === "top" && (
                <Badge className="flex gap-1 whitespace-nowrap bg-amber-500 text-amber-950 shadow-sm hover:bg-amber-500">
                  <Star size={12} />
                  Top Verified
                </Badge>
              )}
              {fundi.premiumLevel === "verified" && (
                <Badge className="flex gap-1 whitespace-nowrap bg-sky-600 text-white shadow-sm hover:bg-sky-600">
                  <Shield size={12} />
                  Verified
                </Badge>
              )}
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {fundi.isEmergency && (
            <Badge variant="outline" className="ml-auto border-red-500/40 bg-red-500/10 text-red-500">
              24/7 Emergency
            </Badge>
          )}
          {fundi.isNearby && (
            <Badge variant="outline" className="flex gap-1 border-emerald-500/40 bg-emerald-500/10 text-emerald-400">
              <MapPin size={12} />
              Nearby
            </Badge>
          )}
        ...
        <p className="line-clamp-3 min-h-18 text-sm text-muted-foreground">{fundi.description}</p>

        <div className="grid grid-cols-2 gap-2 pt-1">
          <Button variant="default" size="sm" className="h-10 rounded-xl bg-primary text-primary-foreground shadow-sm transition-transform group-hover:scale-[1.01]" asChild>
            <a
          ...
          <Button variant="outline" size="sm" className="h-10 rounded-xl border-primary/30 bg-background/60" asChild>
