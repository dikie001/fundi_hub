...
export function ProfileTab({
  ...
  const handleSkillsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSkills(val.split(",").map((s) => s.trim()).filter(Boolean))
  }
  ...
  return (
    <div className="animate-in space-y-6 duration-300 fade-in slide-in-from-bottom-2">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-1 sm:justify-start">
                ...
                <span className="inline-flex items-center gap-1 rounded-lg border border-amber-500/20 bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-600 dark:text-amber-500">
                  <Star className="h-3.5 w-3.5 fill-amber-500 text-amber-500" /> {profile?.rating?.toFixed(1) || "5.0"}
                </span>
              </div>
            </div>
          </div>

          <div className="flex flex-col items-center gap-2.5 w-full md:w-auto md:items-end">
            <Button
            ...
            {updateSuccess && <span className="text-[11px] font-medium text-emerald-500">{updateSuccess}</span>}
          </div>
        </div>
      </Card>
        ...
        <Card className="space-y-4 border border-border/40 bg-card p-6 md:col-span-2">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed bg-muted/10 rounded-lg p-3 border border-border/20 min-h-24 whitespace-pre-wrap">
                {editDesc || "No professional biography written yet. Click 'Edit Profile' to add details."}
              </p>
            </div>
            <div className="space-y-2 pt-1">
              <span className="text-sm font-semibold text-foreground">Specializations</span>
              <div className="flex flex-wrap gap-2">
                {skills.length > 0 ? (
                  ...
                  <span className="text-xs text-muted-foreground italic">No specializations added.</span>
                )}
              </div>
            </div>
          </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b border-border/25 px-6 py-4">
          <div>
            ...
            <CardDescription className="mt-0.5 text-sm text-muted-foreground">
              Real photos of recent customer repairs and installations you completed.
            </CardDescription>
          </div>
          <Button
            size="sm"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Your profile completion score is {completionScore}%. Complete all details to boost your matching priority.
            </p>
          </div>
        </div>
        <Button
      ...
      <Dialog open={isEditProfileOpen} onOpenChange={setIsEditProfileOpen}>
        <DialogContent className="w-full max-w-lg rounded-lg border border-border bg-card p-6 shadow-lg overflow-y-auto max-h-[90vh]">
          <DialogHeader>
          ...
          <form onSubmit={onEditProfileSubmit} className="mt-4 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="edit-title" className="text-sm font-semibold text-foreground">Professional Title</Label>
                <Input
                  id="edit-title"
                  value={editTitle}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="edit-exp" className="text-sm font-semibold text-foreground">Years of Experience</Label>
                <Input
                  id="edit-exp"
                  type="number"
                  value={editYearsExp}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="edit-area" className="text-sm font-semibold text-foreground">Service Coverage Area</Label>
                <Input
                  id="edit-area"
                  value={editArea}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="edit-contact" className="text-sm font-semibold text-foreground">Preferred Contact Method</Label>
                <select
                  id="edit-contact"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="edit-skills" className="text-sm font-semibold text-foreground">Specializations (comma-separated)</Label>
              <Input
                id="edit-skills"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="edit-desc-dialog" className="text-sm font-semibold text-foreground">Professional Biography</Label>
              <textarea
                id="edit-desc-dialog"
                value={editDesc}
            ...
            <DialogFooter className="flex items-center justify-end gap-2 border-t border-border/30 pt-4">
              <Button type="button" variant="ghost" onClick={() => setIsEditProfileOpen(false)} className="h-10 cursor-pointer rounded-lg px-4 text-sm">
                Cancel
              </Button>
              <Button type="submit" disabled={isUpdating} className="h-10 cursor-pointer rounded-lg px-4 text-sm font-semibold">
        ...
        <DialogContent className="w-full max-w-sm rounded-lg border border-border bg-card p-6 shadow-lg">
          ...
          <form onSubmit={handlePortfolioUpload} className="mt-2 space-y-4">
            ...
            <DialogFooter className="flex items-center justify-end gap-2 border-t border-border/30 pt-2">
              <Button type="button" variant="ghost" onClick={() => setIsAddPortfolioOpen(false)} className="h-10 cursor-pointer rounded-lg px-4 text-sm">
                Cancel
              </Button>
              <Button
                type="submit"
              ...
              >
                {isPortfolioUploading ? `Uploading ${portfolioProgress}%` : "Save Work"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
